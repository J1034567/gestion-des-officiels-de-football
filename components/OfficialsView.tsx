...
type SmartFilter =
  ...
  | "available"
  | "missingEmail"
  | "topUnavailableNextWeekend"
  | "assignedWithIncompleteProfile";
...
const ActionCenter: React.FC<{
  ...
  const actions = useMemo(() => {
    ...
    // 1. Missing emails
    const missingEmailOfficials = officials.filter(
      (o) => !o.email || o.email.trim() === ""
    );
    ...
    const assignedWithIncompleteProfile = officials.filter(
      (o) =>
        upcomingMatchOfficialIds.has(o.id) &&
        getIncompleteProfileFields(o).length > 0
    );
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg mb-6 border-l-4 border-yellow-400">
      <h3 className="text-lg font-bold text-white mb-3 flex items-center">
        <AlertTriangleIcon className="h-5 w-5 mr-2 text-yellow-400" /> Centre
        d'Actions
      </h3>
      <ul className="space-y-2">
        {actions.map((action, index) => (
          <li key={index}>
            <button
              onClick={() => onFilterSelect(action.filter)}
              className="text-left text-yellow-300 hover:text-yellow-200 hover:underline text-sm"
            >
              - {action.text}
            </button>
          </li>
...
const StatCard: React.FC<{
  ...
  <div
    onClick={onClick}
    className={`bg-gray-800 p-4 rounded-lg flex items-center cursor-pointer transition-all duration-200 border-2 ${
      isActive
...
const OfficialsView: React.FC<OfficialsViewProps> = (props) => {
  ...
  const [selectedOfficialIds, setSelectedOfficialIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const nextUpcomingMatchDate = useMemo(() => {
    ...
    const upcomingMatchDates = matches
      .filter(
        (match) => new Date(match.matchDate!) >= today && !match.isArchived
      )
  ...
  const handleOpenBulkLocationModal = useCallback(() => {
    const officialsToEdit = officials.filter((o) =>
      selectedOfficialIds.has(o.id)
    );
  ...
  const stats = useMemo(() => {
    ...
    const available = activeOfficials.filter(
      (o) => !isUnavailableOnDate(o, nextUpcomingMatchDate)
    ).length;
    const incomplete = activeOfficials.filter(
      (o) => getIncompleteProfileFields(o).length > 0
    ).length;
  ...
  const filteredOfficials = useMemo(() => {
    ...
    return activeOfficials.filter((o) => {
      ...
      if (
        ...
        return false;
      if (
        smartFilter === "available" &&
        isUnavailableOnDate(o, nextUpcomingMatchDate)
      )
        return false;
      if (smartFilter === "missingEmail" && o.email && o.email.trim() !== "")
        return false;
      if (
        smartFilter === "assignedWithIncompleteProfile" &&
        (!upcomingMatchOfficialIds.has(o.id) ||
          getIncompleteProfileFields(o).length === 0)
      )
        return false;
      ...
      if (debouncedSearch) {
        const normalize = (str: string | null | undefined): string =>
          (str || "")
            ...
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/[/,-]/g, " ");
        const searchKeywords = normalize(debouncedSearch)
          .split(" ")
          .filter(Boolean);
    ...
    if (listSortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (typeof aValue === "string" && typeof bValue === "string") {
          ...
          if (aValue < bValue)
            return listSortConfig.direction === "asc" ? -1 : 1;
          if (aValue > bValue)
            return listSortConfig.direction === "asc" ? 1 : -1;
          return 0;
        }
  ...
  const groupedOfficials = useMemo(() => {
    return filteredOfficials.reduce((acc, official) => {
      ...
      switch (groupBy) {
        ...
        case "profileStatus":
          key =
            getIncompleteProfileFields(official).length > 0
              ? "Profil Incomplet"
              : "Profil Complet";
          break;
        case "location":
        default:
          const location = official.locationId
            ? locationMap.get(official.locationId)
            : null;
          key =
  ...
  useEffect(() => {
    const initialExpandedState = Object.keys(groupedOfficials).reduce(
      (acc, groupName) => {
        acc[groupName] = false;
        return acc;
      },
  ...
  }, [debouncedSearch, availabilityFilter, groupBy, smartFilter]);
            ...
            <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                {permissions.can("create", "official") && (
                  <button
                    onClick={handleAdd}
                    className="flex items-center justify-center bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200"
                  >
            ...
            {!isOfficialUser && (
              <>
                <ActionCenter
                  ...
                  onFilterSelect={handleSmartFilterClick}
                />
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <StatCard
                    title="Officiels Actifs"
                    value={stats.total}
                    icon={<UsersIcon className="h-8 w-8 text-blue-400" />}
                    onClick={() => handleSmartFilterClick("all")}
                    isActive={smartFilter === "all"}
                  />
                  <StatCard
                    title="Disponibles Prochainement"
                    value={stats.available}
                    icon={
                      <CheckCircleIcon className="h-8 w-8 text-green-400" />
                    }
                    onClick={() => handleSmartFilterClick("available")}
                    isActive={smartFilter === "available"}
                  />
                  <StatCard
                    ...
                    onClick={() => handleSmartFilterClick("incomplete")}
                    isActive={smartFilter === "incomplete"}
                  />
                </div>
                {smartFilter !== "all" && (
                  <div className="bg-brand-primary/10 text-brand-primary text-sm p-3 rounded-lg flex justify-between items-center mb-4">
                    <span>
                      ...
                      <span className="font-semibold">
                        {smartFilterLabels[smartFilter]}
                      </span>
                    </span>
                    <button
                      onClick={() => setSmartFilter("all")}
                      className="p-1 rounded-full hover:bg-brand-primary/20"
                  ...
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-4">
                    <div className="flex items-center space-x-2">
                      <span className="text-sm font-medium text-gray-400">
                        Disponibilité{" "}
                        {nextUpcomingMatchDate
                          ? `(le ${nextUpcomingMatchDate.toLocaleDateString(
                              "fr-FR"
                            )})`
                          : ""}
                        :
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <select
                            ...
                            value={sortOrder}
                            onChange={(e) =>
                              setSortOrder(e.target.value as "name" | "count")
                            }
                            className="bg-gray-900 border border-gray-700 rounded-md shadow-sm py-1.5 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary text-sm"
              ...
              <div className="space-y-4">
                {isOfficialUser ? (
                  ...
                  sortedGroupNames.map((groupName) => {
                    ...
                    const areAllInGroupSelected =
                      groupOfficials.length > 0 &&
                      groupOfficials.every((o) =>
                        selectedOfficialIds.has(o.id)
                      );
                    ...
                    const referees = groupOfficials.filter(
                      (o) => !o.category.toLowerCase().includes("délégué")
                    );
                    const delegates = groupOfficials.filter((o) =>
                      o.category.toLowerCase().includes("délégué")
                    );
                    ...
                    return (
                      <div
                        key={groupName}
                        className="bg-gray-800 rounded-lg transition-all duration-300"
                      >
                        <button
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <span className="ml-3 text-sm font-medium text-gray-300 bg-gray-600 px-2.5 py-0.5 rounded-full">
                              {groupOfficials.length}
                            </span>
                          </div>
                          <ChevronDownIcon
                                  ...
                                  renderItem={(official) => {
                                    ...
                                    return (
                                      <OfficialCard
                                        key={official.id}
                                        official={official}
                                        officialLocation={formatLocation(
                                          official.locationId
                                        )}
                                        nextUpcomingMatchDate={
                                          nextUpcomingMatchDate
                                        ...
                                        permissions={permissions}
                                        currentUser={currentUser}
                                        isSelected={selectedOfficialIds.has(
                                          official.id
                                        )}
                                        onSelect={handleSelectOfficial}
                                        isProfileComplete={
                  ...
                  <div className="text-center py-8 bg-gray-800 rounded-lg">
                    <p className="text-gray-400">
                      Aucun officiel ne correspond aux filtres actuels.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-gray-800/90 backdrop-blur-sm border-t border-gray-700 p-4 shadow-lg z-20 animate-fade-in-up">
                <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
                  <p className="text-white font-medium">
                    {selectedOfficialIds.size} officiel(s) sélectionné(s)
                  </p>
                  <div className="flex items-center gap-x-4 gap-y-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center text-sm bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2 px-3 rounded-lg transition-colors"
                      >
                        <LocationPinIcon className="h-5 w-5 mr-2" />{" "}
                        Localisation
                      </button>
                      <button
                    ...
                    <button
                      onClick={() => setSelectedOfficialIds(new Set())}
