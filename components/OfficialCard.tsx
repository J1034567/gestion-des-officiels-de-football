...
const OfficialCard: React.FC<OfficialCardProps> = ({
  ...
  const canManageAvailability = permissions.can(
    "edit",
    "availability",
    official
  );
  ...
  const availabilityTitle = useMemo(() => {
    ...
    return isAvailable
      ? `Disponible le ${dateString}`
      : `Indisponible le ${dateString}`;
  ...
  return (
    <>
      <div
        ...
        className={`bg-gray-900/50 rounded-lg shadow-md p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all duration-200 cursor-pointer ${
          isSelected
          ...
          <div className="ml-4">
            <div className="flex items-center gap-2">
              ...
              {official.position != null && (
                <span
                  className="text-xs font-bold text-cyan-300 bg-cyan-900/50 px-2 py-1 rounded-full"
                  title={`Position: ${official.position}`}
                >
                  #{official.position}
                </span>
              )}
                  ...
                  {canEdit && (
                    <button
                      ...
                      className={`w-full text-left flex items-center px-4 py-2 text-sm text-gray-200 hover:bg-gray-600 ${
                        !canArchive ? "rounded-md" : "rounded-t-md"
                      }`}
                    >
                  ...
                  {canArchive && (
                    <button
                      ...
                      className={`w-full text-left flex items-center px-4 py-2 text-sm text-red-300 hover:bg-red-500 hover:text-white ${
                        !canEdit ? "rounded-md" : "rounded-b-md"
                      }`}
                    >
...
export default React.memo(OfficialCard);
