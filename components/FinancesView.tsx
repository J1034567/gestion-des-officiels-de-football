      ...
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-300">
            Cette note sera ajoutée ou remplacera la note existante pour les{" "}
            {count} paiements sélectionnés.
          </p>
          <textarea
            value={note}
...
const FinancesView: React.FC<FinancesViewProps> = (props) => {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: SortKey;
    direction: "asc" | "desc";
  } | null>({ key: "matchDate", direction: "desc" });
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<AccountingStatus | "all">(
    "all"
  );
  const [dateRange, setDateRange] = useState({ start: "", end: "" });
  const [paymentForDetails, setPaymentForDetails] = useState<Payment | null>(
    null
  );
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter((payment) => {
      if (statusFilter !== "all" && payment.accountingStatus !== statusFilter)
        return false;
      ...
      if (searchTerm) {
        ...
        if (
          !payment.officialName.toLowerCase().includes(lowerTerm) &&
          !payment.matchDescription.toLowerCase().includes(lowerTerm)
        ) {
          return false;
        }
      }
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === "validator") {
          aValue = a.validatedByUserId
            ? userMap.get(a.validatedByUserId) || ""
            : "";
          bValue = b.validatedByUserId
            ? userMap.get(b.validatedByUserId) || ""
            : "";
        } else {
  ...
  const handleExport = async () => {
    const result = await exportPaymentsToExcel(sortedPayments, users);
    if (!result.success) {
      setAlertModalInfo({
  ...
  const stats = useMemo(() => {
    return payments.reduce(
      (acc, p) => {
        ...
        if (p.accountingStatus === AccountingStatus.VALIDATED)
          acc.readyToPayCount++;
        return acc;
      },
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <div className="bg-gray-800 p-4 rounded-lg">
            <p className="text-sm text-gray-400">Total Indemnités Brutes</p>
            <p className="text-2xl font-bold text-white">
              {formatCurrency(stats.totalGross)}
            </p>
          </div>
          <div className="bg-gray-800 p-4 rounded-lg">
            <p className="text-sm text-gray-400">Total IRG Retenu</p>
            <p className="text-2xl font-bold text-white">
              {formatCurrency(stats.totalIrg)}
            </p>
          </div>
          <div className="bg-gray-800 p-4 rounded-lg">
            <p className="text-sm text-gray-400">Total Net à Payer</p>
            <p className="text-2xl font-bold text-brand-primary">
              {formatCurrency(stats.totalNet)}
            </p>
          </div>
          <div className="bg-gray-800 p-4 rounded-lg">
            <p className="text-sm text-gray-400">Paiements Prêts</p>
            <p className="text-2xl font-bold text-white">
              {stats.readyToPayCount}
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-4 rounded-lg mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative flex-grow w-full sm:w-auto">
            ...
            <input
              ...
              placeholder="Rechercher par officiel ou match..."
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            <select
              ...
              className="w-full sm:w-auto bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-white"
            >
              <option value="all">Tous les statuts</option>
              {Object.entries(accountingStatusConfig).map(([key, { text }]) => (
            ...
            <button
              ...
              className="w-full sm:w-auto flex-shrink-0 flex items-center justify-center bg-blue-600 text-white font-bold py-2 px-3 rounded-lg hover:bg-blue-700"
            >
            ...
            <table className="min-w-full">
                ...
                <tr>
                  <th className="p-4">
                    <input
                      type="checkbox"
                      checked={
                        selectedPaymentIds.size === sortedPayments.length &&
                  ...
                  <th
                    ...
                    className="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase"
                  >
                    Officiel
                  </th>
                  <th
                    onClick={() => handleSort("matchDescription")}
                    className="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase"
                  >
                    Match
                  </th>
                  <th
                    ...
                    className="cursor-pointer px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase"
                  >
                    Total Net
                  </th>
                  <th
                    onClick={() => handleSort("accountingStatus")}
                    className="cursor-pointer px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase"
                  >
                    Statut
                  </th>
              ...
              <tbody className="divide-y divide-gray-700">
                {sortedPayments.map((p) => {
                  const statusStyle =
                    accountingStatusConfig[p.accountingStatus];
                  return (
                    <tr
                      key={p.id}
                      className={`transition-colors ${
                        selectedPaymentIds.has(p.id)
                          ? "bg-brand-primary/10"
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-400">
                          {new Date(p.matchDate).toLocaleDateString("fr-FR")}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-300">
                        {p.validatedByUserId
                          ? userMap.get(p.validatedByUserId)
                          : "N/A"}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-400 italic">
                        <div className="flex items-center gap-2">
                          <span className="truncate" title={p.notes || ""}>
                            {p.notes || "Aucune note"}
                          </span>
                          <button
                            onClick={() => setPaymentForDetails(p)}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gray-800/90 backdrop-blur-sm border-t border-gray-700 p-4 shadow-lg z-20 animate-fade-in-up">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <p className="text-white font-medium">
              {selectedPaymentIds.size} paiement(s) sélectionné(s)
            </p>
            <div className="flex items-center gap-x-4">
              <button
                onClick={() => setIsBulkNotesModalOpen(true)}
                className="flex items-center text-sm bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2 px-3 rounded-lg"
              >
                <PencilIcon className="h-4 w-4 mr-2" /> Ajouter une note
              </button>
              <button
