...
// Distance now resolved via hook (precomputed table + fallback + persistence)
...
const AssignmentModal: React.FC<AssignmentModalProps> = ({
  ...
  const [distances, setDistances] = React.useState<Map<string, number | null>>(
    new Map()
  );
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (!match?.stadium?.locationId) {
        if (!cancelled) setDistances(distancesMap);
        return;
      }
      ...
      for (const official of availableOfficials) {
        if (official.locationId) {
          ...
          if (officialLocation) {
            const dist = await getRoundTripBufferedKm(
              officialLocation,
              stadiumLocation
            );
            distancesMap.set(official.id, dist);
          } else {
  ...
  }, [match, availableOfficials, locationMap, getRoundTripBufferedKm]);
        ...
        <div className="p-6 border-b border-gray-700 flex justify-between items-center flex-shrink-0">
          <div className="flex items-center gap-4">
            {onBack && (
              <button
                onClick={onBack}
                className="text-gray-400 hover:text-white p-2 -ml-2 rounded-full hover:bg-gray-700"
                title="Retour"
              >
          ...
          <ul className="space-y-3">
            {filteredOfficials.length > 0 ? (
                    ...
                    <div className="flex items-center space-x-4">
                      {distance !== null && typeof distance !== "undefined" && (
                        <div
                          ...
                          title="Distance aller-retour estimée (incl. 20% de marge)"
                        >
              ...
              <p className="text-center text-gray-400 py-8">
                Aucun officiel disponible ne correspond à votre recherche.
              </p>
            )}
          </ul>
        </div>
