...
import { Match, Official, User, OfficialRole, AccountingStatus, MatchStatus } from '../types';
...
const PlaceholderAssignmentSlot: React.FC<{ role: OfficialRole; onAdd: () => Promise<void>; canEdit: boolean; }> = ({ role, onAdd, canEdit }) => {
  ...
  return (
    ...
    >
      ...
      <span className="px-4 py-2 text-sm font-semibold text-white bg-gray-600 group-hover:bg-gray-500 transition-colors rounded-full flex items-center">
        {isAdding && <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>}
...
const MatchAssignmentsModal: React.FC<MatchAssignmentsModalProps> = ({
  ...
  const isMatchLockedForAccounting = useMemo(() => {
    ...
    return match.accountingStatus === AccountingStatus.VALIDATED || 
  ...
  const { existingAssignments, placeholderRoles } = useMemo(() => {
    ...
    const placeholders = officialRoles.filter(role => !existingRoles.has(role));

    return {
        existingAssignments: existing.sort((a, b) => officialRoles.indexOf(a.role) - officialRoles.indexOf(b.role)),
        placeholderRoles: placeholders.sort((a, b) => officialRoles.indexOf(a) - officialRoles.indexOf(b)),
    };
  }, [match, officialRoles]);
  ...
  const canAssignOfficial = permissions.can('assign', 'official') && isMatchStateEditableForAssignments;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl transform transition-all duration-300 flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center flex-shrink-0">
          <div>
            <h2 className="text-2xl font-bold text-white">Gérer les Désignations</h2>
          ...
          {existingAssignments.map(assignment => {
            ...
            return (
              <OfficialAssignment
                ...
                originalOfficial={officials.find(o => o.id === (assignment.originalOfficialId || null))}
                ...
                onUnassign={assignment.officialId ? () => onUpdateAssignment(match.id, assignment.id, null) : undefined}
// FIX: Changed prop name from onMarkAbsent to onMarkOfficialAbsent to match the component's props interface.
                onMarkOfficialAbsent={onMarkOfficialAbsent ? () => onMarkOfficialAbsent(match.id, assignment.id) : undefined}
                canEdit={canAssignOfficial}
                onPrintMissionOrder={assignment.officialId ? () => onPrintIndividualMissionOrder(match.id, assignment.officialId!) : undefined}
                onSendMissionOrder={assignment.officialId ? () => onSendIndividualMissionOrder(match.id, assignment.officialId!) : undefined}
              />
            );
          })}
        ...
        <div className="p-6 bg-gray-900/50 rounded-b-lg flex justify-between items-center">
            <button
                ...
                title={assignedOfficialsWithEmail.length === 0 ? "Aucun officiel avec email assigné" : "Envoyer les ordres de mission à tous les officiels assignés"}
