...
const MatchesView: React.FC<MatchesViewProps> = ({
  ...
  const [activeGameDayTabs, setActiveGameDayTabs] = useState<
  ...
  const availableGroups = useMemo(() => {
    ...
    return leagueGroups.filter(
      (g) => g.league_id === selectedLeagueId && g.season === currentSeason
    );
  ...
  const smartSummary = useMemo(() => {
    ...
    const unscheduled = matches.filter(
      (m) => m.status === MatchStatus.SCHEDULED && !m.matchDate
    ).length;
  ...
  const { hierarchicalData, sortedLeagueIds } = useMemo(() => {
    ...
    const filteredMatches = matches.filter((match) => {
      ...
      if (
        ...
        match.leagueGroup.league.id !== selectedLeagueId
      )
        return false;
      if (
        selectedLeagueId !== "all" &&
        selectedGroupId !== "all" &&
        match.leagueGroup.id !== selectedGroupId
      )
        return false;
    ...
    for (const leagueId in data) {
      for (const groupId in data[leagueId].groups) {
        ...
        for (const day in groupData.gameDays) {
          ...
          const dayComplete = dayData.matches.filter(
            (m) =>
              m.assignments.length > 0 &&
              m.assignments.every((a) => !!a.officialId)
          ).length;
        ...
        groupData.sortedGameDays = Object.keys(groupData.gameDays)
          .map(Number)
          .sort((a, b) => a - b);
      }
    }
    ...
    for (const leagueId in hierarchicalData) {
      for (const groupId in hierarchicalData[leagueId].groups) {
        if (!activeGameDayTabs[groupId]) {
          const firstGameDay =
            hierarchicalData[leagueId].groups[groupId].sortedGameDays[0];
          if (firstGameDay) {
  ...
  const currentMatch = assignmentContext
    ? matches.find((m) => m.id === assignmentContext.matchId)
    : null;

  const availableOfficials =
    currentMatch && currentMatch.matchDate
      ? officials.filter((official) => {
          if (!official.isActive || official.isArchived) return false;

          const isAlreadyAssignedInThisMatch = currentMatch.assignments.some(
          ...
          const hasDeclaredUnavailability = official.unavailabilities.some(
            (unavailability) => {
          ...
          const isAssignedOnSameDay = matches.some(
            (m) =>
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gray-800 p-4 rounded-lg flex items-center">
          ...
          <div>
            <div className="text-2xl font-bold text-white">
              {smartSummary.unscheduled}
            </div>
            <div className="text-sm text-gray-400">
              Matchs sans date programmée
            </div>
          </div>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg flex items-center">
          ...
          <div>
            <div className="text-2xl font-bold text-white">
              {smartSummary.unscored}
            </div>
            <div className="text-sm text-gray-400">
              Matchs joués sans score enregistré
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 p-4 rounded-lg mb-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <div className="relative xl:col-span-1">
            <label htmlFor="search-term-matches" className="sr-only">
              Rechercher
            </label>
          ...
          <div className="xl:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <select
              ...
              className="bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary text-sm"
            >
            ...
            <select
              ...
              className="bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary"
            >
            ...
            <select
              ...
              className="bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary disabled:opacity-50"
            >
        ...
        {sortedLeagueIds.length > 0 ? (
          sortedLeagueIds.map((leagueId) => {
            ...
            return (
              <div
                key={leagueId}
                className="bg-gray-800 rounded-lg overflow-hidden transition-all duration-300 border border-gray-700/50"
              >
                <button
                ...
                {isExpanded && (
                  <div className="p-4 space-y-6">
                    {Object.values(groups).map(
                      ({ group, gameDays, sortedGameDays, stats }) => {
                        const activeDay =
                          activeGameDayTabs[group.id] || sortedGameDays[0];
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center">
                                ...
                                <ChevronDownIcon
                                  className={`h-5 w-5 text-gray-400 ml-2 transition-transform duration-200 ${
                                ...
                                <div className="border-b border-gray-700">
                                  ...
                                  >
                                    {sortedGameDays.map((day) => {
                                      ...
                                      const dayIndicatorColor = isDayComplete
                                        ? "text-green-400"
                                        : dayStats.complete > 0
                                        ? "text-yellow-400"
                                        : "text-gray-500";
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          {dayStats.total > 0 && (
                                            <span
                                              className={`font-mono text-xs ${dayIndicatorColor}`}
                                            >
                                              ({dayStats.complete}/
                                              {dayStats.total})
                                            </span>
          ...
          <div className="text-center bg-gray-800 p-12 rounded-lg">
            ...
            <p className="mt-2 text-gray-400">
              Essayez d'ajuster vos filtres de recherche ou de créer de nouveaux
              matchs.
            </p>
          </div>
        )}
      ...
      <CreateMatchModal
        ...
        matches={matches}
        isEditable={
          editingMatch ? editingMatch.status === MatchStatus.SCHEDULED : true
        }
        seasons={seasons}
