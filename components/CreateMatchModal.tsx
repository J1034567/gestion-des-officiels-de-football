...
import { Match, Team, Stadium, League, LeagueGroup, TeamSeasonStadium, AccountingStatus, Location } from '../types';
...
interface CreateMatchModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveMatch: (matchData: Omit<Match, 'status' | 'assignments' | 'isSheetSent' | 'hasUnsentChanges' | 'isArchived' | 'createdAt' | 'createdByName' | 'updatedAt' | 'updatedBy' | 'updatedByName'> & { id?: string }) => void;
...
const CreateMatchModal: React.FC<CreateMatchModalProps> = ({ isOpen, onClose, onSaveMatch, matchToEdit, teams, stadiums, leagues, leagueGroups, matches, isEditable, seasons, currentSeason, teamStadiums, locations }) => {
  ...
  const availableGroups = useMemo(() => {
    return leagueGroups.filter(g => g.league_id === selectedLeagueId && g.season === season);
  ...
  const availableTeams = useMemo(() => {
    ...
    if (!selectedGroup || !selectedGroup.teamIds || selectedGroup.teamIds.length === 0) {
  ...
  const selectedStadium = useMemo(() => stadiums.find(s => s.id === stadiumId), [stadiumId, stadiums]);
  ...
  useEffect(() => {
    if (isOpen) {
        if (isEditing) {
            ...
            const firstGroup = leagueGroups.find(g => g.league_id === firstLeagueId && g.season === currentSeason);
  ...
  }, [matchToEdit, isEditing, isOpen, stadiums, leagues, leagueGroups, currentSeason]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!season || !selectedLeagueId || !selectedGroupId || !gameDay || !homeTeamId || !awayTeamId) {
        setError("Veuillez remplir au minimum: Saison, Journée, Ligue, Groupe et les deux Équipes.");
        return;
    }
    if (gameDay < 1) {
    ...
    if (date && matches.some(m => m.homeTeam.id === homeTeamId && m.awayTeam.id === awayTeamId && m.matchDate === date && m.id !== matchToEdit?.id)) {
    ...
    if (!homeTeam || !awayTeam || !leagueGroup || !league) {
        setError("Données de base invalides (équipe, ligue ou groupe non trouvé).");
        return;
    }
    ...
    const matchData = {
        ...
        accountingStatus: matchToEdit?.accountingStatus || AccountingStatus.NOT_ENTERED,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl transform transition-all duration-300">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center">
            <h2 className="text-2xl font-bold text-white">{isEditing ? 'Détails du match' : 'Créer un nouveau match'}</h2>
        ...
        <form onSubmit={handleSubmit}>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                {formDisabled && <p className="text-yellow-300 bg-yellow-900/50 p-3 rounded-md text-sm">Les modifications sont désactivées car le statut du match n'est pas "Prévu".</p>}
                {error && <p className="text-red-400 bg-red-900/50 p-3 rounded-md text-sm">{error}</p>}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        <label htmlFor="season" className="block text-sm font-medium text-gray-300">Saison</label>
                        <select id="season" value={season} onChange={e => setSeason(e.target.value)} disabled={formDisabled} className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700">
                            {seasons.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                    </div>
                     <div>
                        <label htmlFor="gameDay" className="block text-sm font-medium text-gray-300">Journée</label>
                        <input type="number" id="gameDay" value={gameDay} onChange={e => setGameDay(e.target.value ? parseInt(e.target.value) : '')} disabled={formDisabled} min="1" className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700" />
                    </div>
                </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="league" className="block text-sm font-medium text-gray-300">Ligue</label>
                        <select id="league" value={selectedLeagueId} onChange={e => setSelectedLeagueId(e.target.value)} disabled={formDisabled} className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700">
                            <option value="">Sélectionner une ligue</option>
                            {leagues.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
                        </select>
                    </div>
                     <div>
                        <label htmlFor="group" className="block text-sm font-medium text-gray-300">Groupe</label>
                        <select id="group" value={selectedGroupId} onChange={e => setSelectedGroupId(e.target.value)} disabled={formDisabled || !selectedLeagueId} className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <DatePicker id="date" value={date} onChange={setDate} disabled={formDisabled} />
                    </div>
                    <div>
                        <label htmlFor="time" className="block text-sm font-medium text-gray-300">Heure</label>
                         <div className="relative mt-1">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <ClockIcon className="h-5 w-5 text-gray-400" />
                            </div>
                            <input type="time" id="time" value={time} onChange={e => setTime(e.target.value)} disabled={formDisabled} className="block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 pl-10 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700" />
                         </div>
                    </div>
                </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        <label htmlFor="homeTeam" className="block text-sm font-medium text-gray-300">Équipe Domicile</label>
                        <select id="homeTeam" value={homeTeamId} onChange={e => setHomeTeamId(e.target.value)} disabled={formDisabled} className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700">
                            <option value="">Sélectionner une équipe</option>
                            {availableHomeTeams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>
                    </div>
                     <div>
                        <label htmlFor="awayTeam" className="block text-sm font-medium text-gray-300">Équipe Extérieur</label>
                        <select id="awayTeam" value={awayTeamId} onChange={e => setAwayTeamId(e.target.value)} disabled={formDisabled} className="mt-1 block w-full bg-gray-900 border border-gray-700 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm disabled:opacity-50 disabled:bg-gray-700">
                            <option value="">Sélectionner une équipe</option>
                ...
                <div className="relative">
                    <label htmlFor="stadium-search" className="block text-sm font-medium text-gray-300">Stade</label>
                    <div className="relative mt-1">
                        <input id="stadium-search" type="text"
                            placeholder={stadiumId ? (selectedStadium?.name || '') : "Rechercher un stade..."}
                            value={stadiumSearch}
                            onChange={(e) => { setStadiumSearch(e.target.value); setIsStadiumDropdownOpen(true); if (stadiumId) setStadiumId(''); }}
                        ...
                        />
                         {stadiumId && (
                            <button type="button" onClick={() => { setStadiumId(''); setStadiumSearch(''); }} disabled={formDisabled}
                                className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-white disabled:cursor-not-allowed" aria-label="Effacer la sélection">
                 ...
                 {isEditing && matchToEdit.createdAt && (
                    <div className="mt-6 pt-4 border-t border-gray-700 text-xs text-gray-400 space-y-1">
                        <p>Créé le: {new Date(matchToEdit.createdAt).toLocaleString('fr-FR')} par {matchToEdit.createdByName}</p>
                        {matchToEdit.updatedAt && matchToEdit.updatedByName && (
                            <p>Dernière modification: {new Date(matchToEdit.updatedAt).toLocaleString('fr-FR')} par {matchToEdit.updatedByName}</p>
                        )}
                    </div>
                )}
            </div>
            <div className="p-6 bg-gray-900/50 rounded-b-lg flex justify-end">
                ...
                <button type="submit" disabled={formDisabled} className="bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-lg disabled:bg-gray-600 disabled:cursor-not-allowed">
