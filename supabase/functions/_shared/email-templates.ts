...
import { Match, Official, Location } from './types.ts'

/**
 * Generates the complete email content (HTML, text, subject) for a match assignment.
 * This function replicates the original, rich HTML email template.
 *
 * @param match - The full match object with related data.
 * @param officials - An array of official objects assigned to this match.
 * @param isUpdate - A boolean indicating if this is an update to a previous assignment.
 * @returns An object containing the subject, html, and text content for the email.
 */
export const generateMatchSheetHtml = (match: Match, officials: Official[], isUpdate: boolean, locations: Location[]): { subject: string, html: string, text: string } => {
  ...
  // Build a richer structure pairing each assignment with its official (if any)
  const assignmentOfficialPairs = match.assignments
    .map(a => {
      const oid = (a as any).officialId ?? (a as any).official_id;
      const official = officials.find(o => o.id === oid);
      return official ? { assignment: a, official } : null;
    })
    .filter((p): p is { assignment: any; official: Official } => p !== null);

  // Determine the main / coordinating official:
  // Prefer an assignment whose role contains 'Délégué' (case-insensitive), otherwise first available.
  const mainOfficial = assignmentOfficialPairs.find(p =>
    (p.assignment.role || '').toString().toLowerCase().includes('délégué')
  )?.official || assignmentOfficialPairs[0]?.official;

  // Format date/time with Arabic locale (robust against null / invalid)
  let formattedDate = 'غير محدد';
  if (match.matchDate) {
    const parsed = new Date(match.matchDate);
    if (!isNaN(parsed.getTime())) {
      try {
        formattedDate = parsed.toLocaleDateString('ar-DZ', {
          weekday: 'long',
          day: 'numeric',
          month: 'long',
          year: 'numeric'
  ...
  const formattedTime = match.matchTime || 'غير محدد';
...
${match.assignments.map(a => {
    const oid = (a as any).officialId ?? (a as any).official_id;
    const official = officials.find(o => o.id === oid);
    const fullArabic = official ? `${official.firstNameAr || official.firstName || ''} ${official.lastNameAr || official.lastName || ''}`.trim() : 'غير محدد';
    return `• ${translateRole(a.role)}: ${fullArabic || 'غير محدد'}`;
  }).join('\n')}
        ...
        <div class="match-details">
          ...
          <div class="detail-row">
            ...
            <div class="detail-content">
              ...
              <div class="detail-value">${formattedDate} الساعة ${formattedTime}</div>
            </div>
          </div>
